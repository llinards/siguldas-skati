<div>
    ...
    <h2 class="text-h-sm-mob lg:text-h-mob mb-3 leading-none">{{ __('Ko darīt Siguldā') }}</h2>
    <div wire:sort="updateActivityOrder" class="space-y-4">
        @forelse ($activities as $activity)
            ...
            >
                ...
                <!-- Activity Image -->
                <div class="flex items-center justify-center bg-gray-50 p-2 flex-shrink-0">
                    @if ($activity->image)
                        <img
                        ...
                        />
                    @else
                        <div class="flex h-16 w-24 items-center justify-center rounded bg-gray-200">
                            <span class="text-sm text-gray-500">{{ __('Nav') }}</span>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex-1 p-4">
                    ...
                    <p class="text-sm text-gray-500">
                        {{ $activity->getTranslation('modal_heading', app()->getLocale()) }}
                    </p>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 p-4 flex-shrink-0">
                    ...
                    <button
                        wire:click="toggleActive({{ $activity->id }})"
                        class="{{ $activity->is_active ? 'bg-red-100 text-red-800 hover:bg-red-200' : 'bg-green-100 text-green-800 hover:bg-green-200' }} inline-flex items-center rounded-md px-3 py-1 text-sm font-medium transition-colors duration-200"
                    >
                        {{ $activity->is_active ? __('Deaktivizēt') : __('Aktivizēt') }}
                    </button>

                    <!-- Edit Button -->
                    <a
                        href="{{ route('dashboard.activity.edit', $activity->id) }}"
                    ...
                    <button
                        ...
                        wire:confirm="{{ __('Vai esat pārliecināts, ka vēlaties dzēst šo aktivitāti?') }}"
        ...
        @empty
            <div class="py-12 text-center">
                <p class="text-base leading-7.5 md:text-xl xl:text-2xl">
                    @lang('Nav pievienota neviena aktivitāte!')
                </p>
            </div>
        @endforelse
    </div>
    <div class="mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-end">
        <x-btn-primary :href="route('dashboard.activity.add')">
            {{ __('Pievienot jaunu aktivitāti') }}
        </x-btn-primary>
    </div>
</div>
