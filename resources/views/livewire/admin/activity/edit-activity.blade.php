        ...
        <div class="space-y-12">
            <div class="border-b border-gray-900/10 pb-12">
                ...
                <div class="mt-5 grid gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        <label for="modal_heading" class="block text-sm/6 font-medium text-gray-900">
                            {{ __('Loga virsraksts') }}
                        </label>
                        <div class="mt-2">
                            <input
                                id="modal_heading"
                                type="text"
                                wire:model="modal_heading"
                                class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                            />
                            <x-input-error :messages="$errors->get('modal_heading')" class="mt-2" />
                        </div>
                    </div>
                </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:space-x-6">
                            @if (! $image && $activity->image)
                                <div>
                                    ...
                                    />
                                    <p class="mt-1 text-sm text-gray-500">{{ __('Pašreizējais attēls') }}</p>
                                </div>
                            @endif
                            ...
                            @if ($image)
                                <div>
                                    <img
                                        ...
                                        class="h-32 w-auto rounded-md border border-gray-300 object-cover"
                                    />
                                    <p class="mt-1 text-sm text-gray-500">{{ __('Jauns attēls') }}</p>
                                </div>
                            @endif
                ...
                <div class="mt-5">
                    <label class="block text-sm/6 font-medium text-gray-900">{{ __('Loga saturs') }}</label>
                    <div class="mt-2" wire:ignore>
                        <textarea id="activity-modal-content-editor" rows="6"></textarea>
                    </div>
                    <x-input-error :messages="$errors->get('modal_content')" class="mt-2" />
                </div>
            </div>
            <div class="mt-6 flex items-center justify-end gap-x-6">
                <a href="{{ route('dashboard.activities') }}" class="text-sm/6 font-semibold text-gray-900">
                    @lang('Atcelt')
                </a>
...
<script>
    ...
    ClassicEditor.create(document.querySelector('#activity-modal-content-editor'), activityEditorConfig)
        .then((newEditor) => {
            newEditor.model.document.on('change:data', () => {
                $wire.set('modal_content', newEditor.getData());
            });
        })
        .catch((error) => {
            console.error('CKEditor (edit-activity) initialization error:', error);
        });
</script>
@endscript
