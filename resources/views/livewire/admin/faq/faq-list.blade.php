<div>
    ...
    <h2 class="text-h-sm-mob lg:text-h-mob mb-3 leading-none">{{ __('Biežāk uzdotie jautājumi') }}</h2>
    <div wire:sort="updateFaqOrder" class="space-y-4">
        @forelse ($faqs as $faq)
            <div
            ...
            >
                ...
                <div class="flex items-center justify-end gap-2 p-4 flex-shrink-0">
                    ...
                    <button
                        ...
                        class="{{ $faq->is_active ? 'bg-red-100 text-red-800 hover:bg-red-200' : 'bg-green-100 text-green-800 hover:bg-green-200' }} inline-flex items-center rounded-md px-3 py-1 text-sm font-medium transition-colors duration-200"
                    >
                        {{ $faq->is_active ? __('Deaktivizēt') : __('Aktivizēt') }}
                    </button>

                    <!-- Edit Button -->
                    <a
                        href="{{ route('dashboard.faq.edit', $faq->id) }}"
                    ...
                    <button
                        ...
                        wire:confirm="{{ __('Vai esat pārliecināts, ka vēlaties dzēst šo jautājumu?') }}"
        ...
        @empty
            <div class="py-12 text-center">
                <p class="text-base leading-7.5 md:text-xl xl:text-2xl">
                    @lang('Nav pievienots neviens jautājums!')
                </p>
            </div>
        @endforelse
    </div>
    <div class="mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-end">
        <x-btn-primary :href="route('dashboard.faq.add')">
            {{ __('Pievienot jaunu jautājumu') }}
        </x-btn-primary>
    </div>
</div>
