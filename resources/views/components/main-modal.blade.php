<div id="{{ $modalId }}"
    ...
    <div
        class="hs-overlay-animation-target hs-overlay-open:scale-100 hs-overlay-open:opacity-100 scale-95 opacity-0 ease-in-out transition-all duration-200 flex justify-center items-center p-4 min-h-dvh sm:max-w-2xl sm:w-full sm:mx-auto">
        <div
            ...
            <div class="flex justify-between items-center py-3 px-4 border-b border-ss-dark">
                <h3 id="modal-label" class="text-btn lg:text-h-sm-mob leading-none mt-2">
                    {{ $modalHeading }}
                </h3>
                <button class="ml-3" type="button" aria-label="Close" data-hs-overlay="#{{ $modalId }}">
                    <span type="button"
            ...
            @if(isset($modalCTA))
                <div class="flex items-center gap-x-2 py-3 px-4 border-t border-ss-dark">
                    <x-btn-primary href="{{ route('products') }}">@lang('Rezervēt')</x-btn-primary>
                </div>
            @endif
        </div>
    </div>
