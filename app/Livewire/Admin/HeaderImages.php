...
class HeaderImages extends Component
{
    ...
    {
        try {
            $this->headerImageService->updateOrder((int) $id, $position);
            $this->flashMessageService->success(__('Secība atjaunota.'));
        } catch (\Exception $e) {
            $this->errorLogService->logError('Failed to update header image order', $e, []);
    ...
    public function store(): void
    {
        ...
        try {
            ...
            $this->errorLogService->logError('Failed to save header images.', $e, []);
    ...
    {
        try {
            $this->headerImageService->delete($imageId);
            $this->flashMessageService->success(__('Attēls dzēsts!'));
        } catch (\Exception $e) {
            $this->errorLogService->logError('Failed to delete header image', $e, [
                'image_id' => $imageId,
            ]);
    ...
    {
        return view('livewire.admin.header-images', [
    ...
    private function storeImages(): void
    {
        foreach ($this->images as $image) {
            $this->headerImageService->store($image);
        }
    }
